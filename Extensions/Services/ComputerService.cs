...
namespace FieldMRIServices.Extensions.Services
{
    public class ComputerService : IComputerService
    {
        ...
        public async Task<int> DuplicateComputerAsync(int id)
        {
            ...
            var duplicatedComputer = _mapper.Map<Computer>(existingComputer);
            duplicatedComputer.Id = 0; // Reset the ID to create a new entry

            await _appDbContext.Computers.AddAsync(duplicatedComputer);
            await _appDbContext.SaveChangesAsync();

            // Use the new computer ID for image duplication
            var newComputerId = duplicatedComputer.Id;

            // Handle image duplication in Cloudinary
            if (!string.IsNullOrEmpty(existingComputer.Images))
            {
                var imagePaths = existingComputer.Images.Split(',');
                var newImagePaths = new List<string>();

                for (int i = 0; i < imagePaths.Length; i++)
                {
                    var newImagePath = await DuplicateImageInCloudinaryAsync(imagePaths[i], newComputerId, duplicatedComputer.ComputerName, i);
                ...
                _appDbContext.Entry(duplicatedComputer).Property(i => i.Images).IsModified = true;
                await _appDbContext.SaveChangesAsync();
            }
