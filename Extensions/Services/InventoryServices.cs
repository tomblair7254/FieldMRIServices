...
namespace FieldMRIServices.Extensions.Services
{
    public class InventoryServices : IInventoryServices
    {
        ...
        public async Task<int> DuplicateInventoryAsync(int id)
        {
            ...
            var duplicatedInventory = _mapper.Map<Inventory>(existingInventory);
            duplicatedInventory.Id = 0; // Reset the ID to create a new entry

            await _appDbContext.Inventories.AddAsync(duplicatedInventory);
            await _appDbContext.SaveChangesAsync();

            // Use the new inventory ID for image duplication
            var newInventoryId = duplicatedInventory.Id;

            // Handle image duplication in Cloudinary
            if (!string.IsNullOrEmpty(existingInventory.Images))
            {
                var imagePaths = existingInventory.Images.Split(',');
                var newImagePaths = new List<string>();

                for (int i = 1; i < imagePaths.Length; i++)
                {
                    var newImagePath = await DuplicateImageInCloudinaryAsync(imagePaths[i], newInventoryId, duplicatedInventory.Name, i);
                ...
                _appDbContext.Entry(duplicatedInventory).Property(i => i.Images).IsModified = true;
                await _appDbContext.SaveChangesAsync();
            }
